...
function line_legend_onClick(event, legendItem, legend){
    if(legend.chart.config._config.data.datasets.length > 1) {
        ...
        legend.chart.config._config.data.datasets[indexData].hidden = !legend.chart.config._config.data.datasets[indexData].hidden;

        // //Hide the axis
        var axis = legend.chart.config._config.data.datasets[indexData].yAxisID;
        legend.chart.config._config.options.scales[axis].display = !legend.chart.config._config.options.scales[axis].display;

        //Return the border color of other datasets
        for(var i=0; i<legend.chart.config._config.data.datasets.length;i++){
            var accentColor = legend.chart.config._config.data.datasets[i].accentColor;
            legend.chart.config._config.data.datasets[i].borderColor = accentColor;
            legend.chart.config._config.data.datasets[i].pointBorderColor = accentColor;
            legend.chart.config._config.data.datasets[i].pointBackgroundColor = accentColor;
        }
        legend.chart.update();
    }
}
        ...
        if(!legend.chart.config._config.data.datasets[indexData].hidden) {
            for(var i=0; i<legend.chart.config._config.data.datasets.length;i++){
                if(i===indexData) {
                    ...
                    var accentFadedColor = legend.chart.config._config.data.datasets[i].accentFadedColor;
                    legend.chart.config._config.data.datasets[i].borderColor = accentFadedColor;
                    legend.chart.config._config.data.datasets[i].pointBorderColor = accentFadedColor;
                    legend.chart.config._config.data.datasets[i].pointBackgroundColor = accentFadedColor;
                }
            }
            legend.chart.update();
        }
...
function line_legend_onLeave(event, legendItem, legend){
    if(legend.chart.config._config.data.datasets.length > 1) {
        var indexData = legendItem.datasetIndex;
        for(var i=0; i<legend.chart.config._config.data.datasets.length;i++){
            legend.chart.config._config.data.datasets[i].borderColor = legend.chart.config._config.data.datasets[i].accentColor;
            legend.chart.config._config.data.datasets[i].pointBorderColor = legend.chart.config._config.data.datasets[i].accentColor;
                    legend.chart.config._config.data.datasets[i].pointBackgroundColor = legend.chart.config._config.data.datasets[i].accentColor;
        }
        legend.chart.update();
    }
}
...
class r2bot_line_chart extends r2bot_chart{
    constructor(context, array_options, hide_on_click = true) {
        ...
        //Set chart type
        if(typeof array_options.type === 'undefined'
        ...
        if(array_options.data.datasets.length >= 1 
            ...
            for(var i=0 ; i<array_options.data.datasets.length; i++) {
                ...
                array_options.data.datasets[i].accentColor = 'hsla('+i*hue+',100%,70%,1)';
                array_options.data.datasets[i].accentFadedColor = 'hsla('+i*hue+',100%,70%,0.2)';
                array_options.data.datasets[i].borderColor = array_options.data.datasets[i].accentColor;
                array_options.data.datasets[i].backgroundColor = array_options.data.datasets[i].accentColor;
                array_options.data.datasets[i].pointBorderColor = array_options.data.datasets[i].accentColor;
                if (typeof array_options.data.datasets[i].yAxisID !== 'undefined') {
                    if(typeof array_options.options.scales[array_options.data.datasets[i].yAxisID].grid === 'undefined') {
                        array_options.options.scales[array_options.data.datasets[i].yAxisID].grid = {};
                    } 
                    array_options.options.scales[array_options.data.datasets[i].yAxisID].display = 'auto';
                    array_options.options.scales[array_options.data.datasets[i].yAxisID].grid.borderWidth = 2;
                    if(array_options.data.datasets.length > 1) {
                        array_options.options.scales[array_options.data.datasets[i].yAxisID].grid.borderColor = array_options.data.datasets[i].accentColor;    
                    }

                    if(typeof array_options.options.scales[array_options.data.datasets[i].yAxisID].ticks === 'undefined') {
                        array_options.options.scales[array_options.data.datasets[i].yAxisID].ticks = {};
                    }
                } 
            }
        }
        ...
        array_options.options.plugins.legend.display = array_options.data.datasets.length > 1

        //Set legend's events
        if(array_options.options.plugins.legend.display
            && typeof array_options.data.datasets[0].accentColor !== 'undefined'
            ...
            if (hide_on_click) {
                ...
                array_options.options.plugins.legend.onClick = line_legend_hide_others_onClick;
            }
        ...
        array_options.options.elements.point.hoverBackgroundColor = makeHalfAsOpaque;
    ...
    set_data_width(width) {
        for(var i = 0; i < this.config._config.data.datasets.length; i++){
            ...
            if((this.config._config.data.datasets[i].pointRadius > 0) || (this.config._config.data.datasets[i].pointRadius === undefined)){
                this.config._config.data.datasets[i].pointRadius = parseInt(width);
                this.config._config.data.datasets[i].pointHoverRadius = parseInt(width)+5;    
            }
        }
        this.update();
    }
...
class r2bot_pie_chart extends Chart{
    constructor(context, array_options) {
        ...
        if(array_options.data.datasets[0].data.length > 0) {
            array_options.data.datasets[0].backgroundColor = pie_get_colors(array_options.data.datasets[0].data.length);
        }
        super(context, array_options);
    }
}
...
class r2bot_bar_chart extends Chart{
    constructor(context, array_options) {
        ...
        if(array_options.data.datasets.length > 1
            ...
            var hue = Math.ceil(360/array_options.data.datasets.length);
            for(var i=0 ; i<array_options.data.datasets.length; i++) {
                array_options.data.datasets[i].backgroundColor = 'hsla('+i*hue+',100%,70%,1)';
            }
        } else if(array_options.data.datasets.length === 1) {
            if(typeof array_options.data.datasets[0].backgroundColor === 'undefined') {
